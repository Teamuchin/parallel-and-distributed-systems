...
void find_mcs_and_index(const int* smArr, int size, int* max_val, int* max_index) {
    *max_val = smArr[0];
    *max_index = 0;
...
int main() {
    ...
    int num_threads = 2;  // Thread number
    ...
    find_mcs_and_index(ex_s_m, ex_size, &ex_mcs_value, &ex_mcs_end_index);
    ...
    find_mcs_and_index(s_m, size, &mcs_value, &mcs_end_index);
    ...
    printf("For array with size: %d and random values between -10000 and 10000:\n", size);    
    printf("MCS Value: %d\n", mcs_value);
